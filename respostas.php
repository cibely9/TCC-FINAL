...
$sql = "SELECT a.*, c.foto as foto_animal, c.nome as nome_animal
FROM adotantes a
left join cachorros c on c.id=a.cachorro_id
...
<script>
    function openModal(data) {
        ...
        if (data.foto_animal  && data.foto_animal  !== "") {
            image.src = "uploads/" + data.foto_animal ; // Ajuste conforme o caminho real
        } else {
            image.src = "uploads/default.jpg"; // Imagem padrão caso não tenha foto
        }

        // Setando os dados no modal
        name.textContent = data.nome_animal;
